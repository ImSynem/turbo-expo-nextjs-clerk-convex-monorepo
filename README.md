...
If you don't have `bun` installed, run `npm install --global bun`.
...
#### Additional Clerk & Convex Configuration

1. **Disable password signup in Clerk:**
   - Go to Clerk Dashboard > Configure > User & Authentication
   - Disable "Sign up with password"

2. **Configure JWT template:**
   - Go to Clerk Dashboard > Configure > JWT Templates
   - Click "Add new template"
   - Select "Convex" template
   - Copy the Issuer URL and save the template

3. **Set Clerk Issuer in Convex:**
   - Go to Convex Dashboard > Settings > Environment Variables
   - Add `CLERK_ISSUER_URL` with the copied Issuer URL

4. **Get Convex HTTP Actions URL:**
   - Go to Convex Dashboard > Settings > URL & Deploy Key
   - Copy and save the "HTTP Actions URL"

5. **Configure Clerk webhook:**
   - Go to Clerk Dashboard > Configure > Webhooks
   - Click "New endpoint"
   - Set URL as: `{HTTP Actions URL}/clerk-users-webhook`
   - Select the appropriate events (typically user.created, user.updated, user.deleted)
   - Save the webhook

6. **Configure Sentry (optional):**
   - Set up a Sentry project
   - Add `EXPO_PUBLIC_SENTRY_DSN` environment variable to your app configuration
